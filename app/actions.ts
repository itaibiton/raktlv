...
export const signUpAction = async (formData: FormData) => {
  ...
  // Get the current lang from the request path or headers
  const url = new URL((await headers()).get("referer") || "");
  const pathParts = url.pathname.split('/');
  const lang = pathParts[1] || 'en'; // Default to 'en' if no lang found
  ...
  const { error } = await supabase.auth.signUp({
    ...
    password,
    options: {
      emailRedirectTo: `${origin}/${lang}/auth/callback?redirect_to=/`,
    },
  });
